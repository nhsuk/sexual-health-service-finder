  ...
  describe('happy path', () => {
    ...
    describe('telephone number formatting', () => {
      it('should format the telephone number', () => {
        const telephoneNumber = '0800 123 4567';
        const contacts = [{ OrganisationContactMethodType: 'Telephone', OrganisationContactValue: telephoneNumber.replace(/ /g, '') }];
        const input = { Contacts: JSON.stringify(contacts) };
        const mappedContacts = mapContacts(input);

        expect(mappedContacts.telephoneNumber).to.equal(telephoneNumber);
      });
    });
  });
