...
describe('addTableMarkup', () => {
  it('should add padding to columns with fewer sessions', () => {
    ...
    expect(result[6].padding).to.equal(1);
  });

  it('should add padding of one less than max sessions for closed day (no sessions)', () => {
    const times = [
      { day: 'Monday', openingTimes: [] },
      { day: 'Tuesday', openingTimes: [{ opens: '9am', closes: '3pm' }, { opens: '4pm', closes: '9pm' }] },
    ];
  ...
  it('should handle missing openingTimes on a day', () => {
    const times = [
      { day: 'Monday' },
